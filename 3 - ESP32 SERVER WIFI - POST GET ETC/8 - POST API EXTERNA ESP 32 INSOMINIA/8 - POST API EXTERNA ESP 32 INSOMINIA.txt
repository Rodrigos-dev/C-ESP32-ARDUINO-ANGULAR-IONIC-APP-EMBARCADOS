    ...
    while (client.connected()) {
      if (client.available()) {
        ...
        if (c == '\n' && currentLineIsBlank) {
          ...
          }else if (method == "POST" && path == "/externalDataCreate") {
            handlePostExternalData(client);
          } else {
...
void getDataFromExternalAPI(WiFiClient &client) {
  ...
  if (httpResponseCode > 0) {
    ...
    if (!error) {
      ...
      // Apaga o LED se userId for 1
      if (userId == 1) {
        digitalWrite(4, LOW);  // Apaga o LED
      }

      // Envia a resposta HTTP completa com os dados recebidos da API
      client.println("HTTP/1.1 200 OK");
      client.println("Content-Type: application/json");
...
//post em api externa
void handlePostExternalData(WiFiClient &client) {
  // Lê todo o corpo da requisição
  String body = "";
  while (client.available()) {
    char c = client.read();
  ...
  if (error) {
    ...
    // Obtém os valores de title, body e userId
    ...
    if (httpResponseCode > 0) {
      ...
      client.println("Connection: close");
      client.println();
      client.println("Failed to post data to external API");
    }
    http.end();
  }
}